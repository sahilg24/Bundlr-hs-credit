...
app.post('/', upload.any(), async (req, res) => {
    ...
    */
    ...
    /* Price is similarly given in Winston */
    const price = await bundlr.getPrice(tx.size) / Math.pow(10,12);
    
    if (price > balance) {
        /* 
            The wallet can be funded with the async function bundlr.fund(). The amount funded should
            be given in Winston as well.
        */

        res.json({message: `The file upload costs ${price} AR, but the wallet only has ${balance} AR available.`})
    } else {
        await tx.sign();
        const id = tx.id;
        await tx.upload();
